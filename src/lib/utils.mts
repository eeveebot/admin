  ...
  try {
    ...
    for (const line of lines) {
      ...
      const rssMatch = line.match(
        /^router_memory_usage_bytes\{type="rss"\}\s+(\d+)/
      );
      if (rssMatch) {
      ...
      const heapMatch = line.match(
        /^router_memory_usage_bytes\{type="heap_used"\}\s+(\d+)/
      );
      if (heapMatch) {
      ...
      const messageTimingMatch = line.match(
        /^router_message_processing_seconds_bucket\{[^}]*le="([^"]+)"\}\s+(\d+)/
      );
      ...
      const commandTimingMatch = line.match(
        /^router_command_processing_seconds_bucket\{[^}]*le="([^"]+)"\}\s+(\d+)/
      );
      ...
      const messageTimeSumMatch = line.match(
        /^router_message_processing_seconds_sum\s+(\d+(?:\.\d+)?)/
      );
      ...
      const messageTimeCountMatch = line.match(
        /^router_message_processing_seconds_count\s+(\d+)/
      );
      if (messageTimeCountMatch) {
        result.message_processing_count = parseInt(
          messageTimeCountMatch[1],
          10
        );
      }

      const commandTimeSumMatch = line.match(
        /^router_command_processing_seconds_sum\s+(\d+(?:\.\d+)?)/
      );
      ...
      const commandTimeCountMatch = line.match(
        /^router_command_processing_seconds_count\s+(\d+)/
      );
