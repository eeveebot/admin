  ...
  void (async () => {
    try {
      ...
      if (!apiToken || !apiUrl) {
        ...
        metrics.recordNatsPublish('list_bot_modules_config_error_response');
      ...
      try {
        ...
        const responseTopic = `chat.message.outgoing.${data.platform}.${data.instance}.${data.channel}`;
        void nats.publish(responseTopic, JSON.stringify(responseMessage));
        metrics.recordNatsPublish('list_bot_modules_connection_error_response');
        metrics.recordCommand(data.platform, data.network || 'unknown', data.channel, 'connection_error');
        return;
      }
      ...
      if (!response.ok) {
        ...
        const responseTopic = `chat.message.outgoing.${data.platform}.${data.instance}.${data.channel}`;
        void nats.publish(responseTopic, JSON.stringify(responseMessage));
        metrics.recordNatsPublish('list_bot_modules_api_error_response');
      ...
      try {
        ...
        const responseTopic = `chat.message.outgoing.${data.platform}.${data.instance}.${data.channel}`;
        void nats.publish(responseTopic, JSON.stringify(responseMessage));
        metrics.recordNatsPublish('list_bot_modules_parse_error_response');
        metrics.recordCommand(data.platform, data.network || 'unknown', data.channel, 'parse_error');
        return;
      }
      ...
      const responseTopic = `chat.message.outgoing.${data.platform}.${data.instance}.${data.channel}`;
      void nats.publish(responseTopic, JSON.stringify(responseMessage));
      metrics.recordNatsPublish('list_bot_modules_response');
      ...
      try {
        ...
        metrics.recordNatsPublish('list_bot_modules_exception_response');
      } catch (sendError) {
